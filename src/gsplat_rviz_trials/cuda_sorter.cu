...
#define CUDA_CHECK(call)                                                        \
  do {                                                                          \
    cudaError_t _e = (call);                                                    \
    if (_e != cudaSuccess) {                                                    \
      fprintf(stderr, "[gsplat/cuda] %s:%d  %s: %s\n",                        \
...
bool CudaSorter::sort(const float cam_fwd[3], uint32_t n)
{
  ...
  // 1. Compute per-splat depth keys in parallel.
  ...
  // 2. CUB descending radix sort: depth keys + identity → sorted float indices.
  ...
  // 3. Copy sorted indices device → pinned host buffer.
  CUDA_CHECK(cudaMemcpyAsync(
    h_vals_out, d_vals_out, n * sizeof(float),
    cudaMemcpyDeviceToHost, s));

  // 4. Wait for all GPU work to complete before the CPU reads h_vals_out.
